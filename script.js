const STORAGE_KEY = "telugu_family_tree_data_v32";
const FOCUS_KEY = "telugu_family_tree_focus_v32";
...
// Master Family Dataset embedded directly to ensure instant mobile loading
const defaultFamily = [
...
let family = loadInitialFamily();
let focusPersonId = loadInitialFocus();

function loadInitialFamily() {
...
function loadInitialFocus() {
  ...
  if (saved && family.some(m => m.id === saved)) return saved;
  const arjun = family.find(m => m.name.toLowerCase() === "arjun");
  return arjun ? arjun.id : (family[0]?.id || "1788499738062");
}
...
function clearStorageAndReset() {
  if (confirm("Reset tree to default members?")) {
    ...
    focusPersonId = family.find(m => m.name.toLowerCase() === "arjun")?.id || family[0].id;
    persistFamilyData();
    renderTree();
    updateHeaderInputs();
    resetZoom();
